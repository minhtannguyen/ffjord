...
if __name__ == '__main__':
    ...
    # model
    latent_dim_eff = (latent_dim + latent_dim * 2) if (args.sup and args.cond) else latent_dim
    ...
    if args.cond:
        ...
        conditioner_pred = LinearZeros(n_sys_params, latent_dim * 2).to(device)
        params = params + list(conditioner.parameters()) + list(conditioner_pred.parameters())
    ...
    try:
        for itr in range(1, args.niters + 1):
            ...
            if args.sup and args.cond: 
                z0_sys = conditioner_pred(pred_theta)
                z0 = torch.cat((z0, z0_sys), dim=1)
