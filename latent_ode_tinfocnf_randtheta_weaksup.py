...
parser.add_argument('--weaksup', type=eval, default=False)
...
if __name__ == '__main__':
    ...
    if args.cond:
        conditioner = LinearZeros(n_sys_params, latent_dim * 2).to(device)
    ...
    try:
        for itr in range(1, args.niters + 1):
            ...
            if args.weaksup:
                qz0_mean, qz0_logvar = out[:, :(latent_dim-2)], out[:, (latent_dim-2):-4]
                qz0_a_mean, qz0_a_logvar = out[:,-4:-3], out[:,-3:-2]
                qz0_b_mean, qz0_b_logvar = out[:,-2:-1], out[:,-1:]
                ...
                z0 = torch.cat((z0, z0_a, z0_b), dim=1)
            else:
            ...
            if args.cond:
                ...
                pz0_mean = pz0_logvar = torch.zeros([z0.size()[0],  z0.size()[1] - 2]).to(device)
                pz0_a_mean = torch.tensor(a_range[0]).repeat(z0.size()[0],1).to(device)
                pz0_a_logvar = (2 * torch.log(torch.tensor(a_range[1]))).repeat(z0.size()[0],1).to(device)
                pz0_b_mean = torch.tensor(b_range[0]).repeat(z0.size()[0],1).to(device)
                pz0_b_logvar = (2 * torch.log(torch.tensor(b_range[1]))).repeat(z0.size()[0],1).to(device)
            else:
                pz0_mean = pz0_logvar = torch.zeros(z0.size()).to(device)
            
            if args.weaksup:
                analytic_kl = normal_kl(qz0_mean, qz0_logvar, pz0_mean, pz0_logvar).sum(-1) + normal_kl(qz0_a_mean, qz0_a_logvar, pz0_a_mean, pz0_a_logvar).sum(-1) + normal_kl(qz0_b_mean, qz0_b_logvar, pz0_b_mean, pz0_b_logvar).sum(-1)
            else:
