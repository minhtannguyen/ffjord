...
def add_noise_concat(x):
    ...
    """
    if args.add_noise:
        aug_signal = torch.zeros(args.concat_size, x.shape[1], x.shape[2]).to(x)
        mask_x = torch.ones_like(x)
        mask_aug = torch.bernoulli(torch.ones_like(aug_signal) * args.drop_noise_ratio)
        mask = torch.cat((mask_x, mask_aug),0)
        
        x = torch.cat((x, aug_signal),0)
