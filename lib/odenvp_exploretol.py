...
class StackedCNFLayers(layers.SequentialFlow):
    def __init__(
        ...
        if squeeze:
            ...
            post = [layers.CNF(_make_odefunc(after_squeeze_size), **cnf_kwargs) for _ in range(n_blocks-1)]
            post.append(layers.CNF(_make_odefunc(after_squeeze_size), **cnf_kwargs_hightol))
            chain += pre + [layers.SqueezeLayer(2)] + post
        else:
            chain += [layers.CNF(_make_odefunc(initial_size), **cnf_kwargs_hightol) for _ in range(n_blocks)]
