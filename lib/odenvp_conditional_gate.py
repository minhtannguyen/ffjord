...
class ODENVP(nn.Module):
    ...
    def _logdensity(self, x, logpx=None):
        ...
        for idx in range(len(self.transforms)):
            x, _logpx, atol, rtol = self.transforms[idx].forward(x, _logpx)
            ...
            atol_list += atol; rtol_list += rtol
        
        out = [o.view(o.size()[0], -1) for o in out]
        out = torch.cat(out, 1)
        
        return (out, atol_list, rtol_list) if logpx is None else (out, _logpx, atol_list, rtol_list)
    ...
    def _generate(self, z, logpz=None):
        ...
        z_prev, _logpz, atol, rtol = self.transforms[-1](zs[-1], _logpz, reverse=True)
        atol_list += atol; rtol_list += rtol
        for idx in range(len(self.transforms) - 2, -1, -1):
            z_prev = torch.cat((z_prev, zs[idx]), dim=1)
            z_prev, _logpz, atol, rtol = self.transforms[idx](z_prev, _logpz, reverse=True)
            atol_list += atol; rtol_list += rtol
        return (z_prev, atol_list[::-1], rtol_list[::-1]) if logpz is None else (z_prev, _logpz, atol_list[::-1], rtol_list[::-1])
...
class StackedCNFLayers(layers.SequentialFlow_Gate):
    def __init__(
        ...
        if squeeze:
            ...
            pre = [layers.CNF_Gate(_make_odefunc(initial_size), c, **cnf_kwargs) for _ in range(n_blocks)]
            ...
            c, h, w = initial_size
            chain += [layers.CNF_Gate(_make_odefunc(initial_size), c, **cnf_kwargs) for _ in range(n_blocks)]
