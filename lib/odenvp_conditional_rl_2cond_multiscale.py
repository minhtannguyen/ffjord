...
class ODENVP(nn.Module):
    ...
    def __init__(
        ...
        # for conditional
        C = int(np.prod(input_size[1:]) * self.condition_ratio//2)
        if self.cond_nn == "linear":
            self.project_ycond = modules.LinearZeros(self.y_class, 2 * C)
            self.project_ycond_color = modules.LinearZeros(self.y_color, 2 * C)
            self.project_class = modules.LinearZeros(C, self.y_class)
            self.project_color = modules.LinearZeros(C, self.y_color)
        elif self.cond_nn == "mlp":
#             self.project_ycond = modules.MultiLinearZeros(self.y_class, C, 2 * C)
#             self.project_ycond_color = modules.MultiLinearZeros(self.y_color, C, 2 * C)
            self.project_ycond = modules.LinearZeros(self.y_class, 2 * C)
            self.project_ycond_color = modules.LinearZeros(self.y_color, 2 * C)
            self.project_class = modules.MultiLinearZeros(C, C//2, self.y_class)
            self.project_color = modules.MultiLinearZeros(C, C//2, self.y_color)
        ...
        if dropout_rate > 0:
            ...
            self.dropout_color = nn.Dropout(p=dropout_rate)
    ...
    def _prior_color(self, y_onehot=None):
        # compute the mean and std of the gaussian used to compute logpz
        h = self.prior_h_color.detach().clone()
        
        assert torch.sum(h) == 0.0
        assert y_onehot is not None
        
        yp = self.project_ycond_color(y_onehot)
        h = yp + h
    ...
    def _logdensity(self, x, logpx=None):
        ...
        for idx in range(len(self.transforms)):
            ...
            if idx < len(self.transforms) - 1:
                d = x.size(1) // 2
                d_sup = int(d*(self.condition_ratio+1))
                x, factor_out, factor_out_unsup = x[:, :d], x[:, d:d_sup], x[:, d_sup:]
            else:
                # last layer, no factor out
                d = x.size(1)
                d_sup = int(d*self.condition_ratio)
                factor_out, factor_out_unsup = x[:, :d_sup], x[:, d_sup:]
            out.append(factor_out); out_unsup.append(factor_out_unsup)
            atol_list += atol; rtol_list += rtol; logpa_list += logp_actions; nfe_list += nfe
        
        out = [o.view(o.size()[0], -1) for o in out]
        
        out_unsup = [o.view(o.size()[0], -1) for o in out_unsup]
        
        return (out, out_unsup, atol_list, rtol_list, logpa_list, nfe_list) if logpx is None else (out, out_unsup, _logpx, atol_list, rtol_list, logpa_list, nfe_list)
...
class StackedCNFLayers(layers.SequentialFlow_Gate):
    def __init__(
        ...
        def _make_odefunc(size):
            ...
            f = layers.ODEfunc_rl(net)
            return f
        
        cnf_kwargs_hightol={"T": cnf_kwargs['T'], "train_T": cnf_kwargs['train_T'], "regularization_fns": cnf_kwargs['regularization_fns'], "solver": cnf_kwargs['solver'], "atol": cnf_kwargs['atol'], "rtol": cnf_kwargs['rtol'], "scale": cnf_kwargs['scale']*cnf_kwargs['scale_fac'], "scale_fac": cnf_kwargs['scale_fac'], "scale_std": cnf_kwargs['scale_std'], "gate": cnf_kwargs['gate']}
        if squeeze:
            c, h, w = initial_size
            after_squeeze_size = c * 4, h // 2, w // 2
            pre = [layers.CNF_Gate(_make_odefunc(initial_size), c, **cnf_kwargs_hightol)]
            for _ in range(n_blocks - 1):
                pre.append(layers.CNF_Gate(_make_odefunc(initial_size), c, **cnf_kwargs))
            post = [layers.CNF_Gate(_make_odefunc(after_squeeze_size), c*4, **cnf_kwargs) for _ in range(n_blocks)]
            chain += pre + [layers.SqueezeLayer(2)] + post
        else:
            c, h, w = initial_size
            chain += [layers.CNF_Gate(_make_odefunc(initial_size), c, **cnf_kwargs_hightol)]
            chain += [layers.CNF_Gate(_make_odefunc(initial_size), c, **cnf_kwargs) for _ in range(n_blocks-1)]
