...
class CNF_augment(nn.Module):
    def __init__(self, odefunc, T=1.0, train_T=False, regularization_fns=None, solver='dopri5', atol=1e-5, rtol=1e-5, concat_size=3):
        super(CNF_augment, self).__init__()
        if train_T:
    ...
    def forward(self, z, logpz=None, integration_times=None, reverse=False):
        ...
        z = torch.cat((z, torch.zeros(z.shape[0], self.concat_size, z.shape[2], z.shape[3]).to(z)), 1)
        if logpz is None:
        ...
        z_t = z_t[:,0:(-self.concat_size),:,:]
        # TODO: still need to adjust logpz_t
