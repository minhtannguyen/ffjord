...
class StackedCNFLayers(layers.SequentialFlow):
    def __init__(
        ...
        cnf_kwargs_hightol={"T": cnf_kwargs['T'], "train_T": cnf_kwargs['train_T'], "regularization_fns": cnf_kwargs['regularization_fns'], "solver": cnf_kwargs['solver'], "atol": cnf_kwargs['atol']*scale_tol, "rtol": cnf_kwargs['rtol']*scale_tol}
        if squeeze:
            c, h, w = initial_size
