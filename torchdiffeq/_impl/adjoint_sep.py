...
class OdeintAdjointMethod(torch.autograd.Function):
    ...
    def forward(ctx, *args):
        ...
        y0, func, t, flat_params, rtol_forward, atol_forward, rtol_reverse, atol_reverse, method, options = \
            args[:-9], args[-9], args[-8], args[-7], args[-6], args[-5], args[-4], args[-3], args[-2], args[-1]

        ctx.func, ctx.rtol_forward, ctx.atol_forward, ctx.rtol_reverse, ctx.atol_reverse, ctx.method, ctx.options = func, rtol_forward, atol_forward, rtol_reverse, atol_reverse, method, options

        with torch.no_grad():
            ans = odeint(func, y0, t, rtol=rtol_forward, atol=atol_forward, method=method, options=options)
    ...
    def backward(ctx, *grad_output):
        ...
        ans = tuple(ans)
        func, rtol_forward, atol_forward, rtol_reverse, atol_reverse, method, options = ctx.func, ctx.rtol_forward, ctx.atol_forward, ctx.rtol_reverse, ctx.atol_reverse, ctx.method, ctx.options
        n_tensors = len(ans)
        ...
        with torch.no_grad():
            ...
            for i in range(T - 1, 0, -1):
                ...
                aug_ans = odeint(
                    ...
                    torch.tensor([t[i], t[i - 1]]), rtol=rtol_reverse, atol=atol_reverse, method=method, options=options
                )
            ...
            return (*adj_y, None, time_vjps, adj_params, None, None, None, None, None, None, None)


def odeint_adjoint_sep(func, y0, t, rtol_forward=1e-6, atol_forward=1e-12, rtol_reverse=1e-6, atol_reverse=1e-12, method=None, options=None):
    ...
    ys = OdeintAdjointMethod.apply(*y0, func, t, flat_params, rtol_forward, atol_forward, rtol_reverse, atol_reverse, method, options)
